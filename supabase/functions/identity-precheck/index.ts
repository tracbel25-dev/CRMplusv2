...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (body?.action === 'release') {
      ...
      if (reservationToken.length < 24) return reply({ ok: true, released: false });
      const { data, error } = await admin().rpc('release_signup_identity', { reservation_token: reservationToken });
