...
alter table public.mp_subscriptions
  add column if not exists dedicated_trial_plan boolean not null default false;

create unique index if not exists mp_one_dedicated_trial_plan
  on public.mp_subscriptions(preapproval_plan_id)
  where dedicated_trial_plan = true and preapproval_plan_id is not null;
