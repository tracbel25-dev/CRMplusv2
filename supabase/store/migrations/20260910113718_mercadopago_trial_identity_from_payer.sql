...
declare
  ...
  document_digits text := regexp_replace(coalesce(provider_document, ''), '[^0-9]', '', 'g');
  secret_key bytea;
  dh text;
begin
  ...
  if document_digits = '' or not private.valid_trial_document(document_digits) then return false; end if;
  ...
  perform pg_advisory_xact_lock(hashtextextended('mp-trial-document:' || dh || ':' || s.app_id, 0));
  ...
  if exists (
    ...
    where t.app_id = s.app_id and (
      t.account_id = s.account_id or t.user_id = uid or t.document_hash = dh
    )
...
revoke all on function public.mp_register_checkout_trial(uuid,text,text,text,timestamptz) from public, anon, authenticated;
grant execute on function public.mp_register_checkout_trial(uuid,text,text,text,timestamptz) to service_role;
