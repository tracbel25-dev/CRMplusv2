...
create unique index if not exists orders_tenant_request_key_uq
  on public.orders (tenant_key, request_key)
  ...
  loop
    ...
    loop
      ...
      update public.order_lines
        set done = case
          when jsonb_typeof(line_item->'done') = 'boolean' then (line_item->>'done')::boolean
          else done
        end
...
drop trigger if exists trg_sync_artemis_public_orders_from_workspace on public.workspace_state;
create trigger trg_sync_artemis_public_orders_from_workspace
after insert or update of data on public.workspace_state
for each row execute function public.sync_artemis_public_orders_from_workspace();

drop function if exists public.create_public_order(text,text,uuid,text,text,text,text,text,jsonb);
...
begin
  ...
  insert into public.orders (
    ...
    v_fee, 0, false, false, trim(p_payment_method), p_request_key
  );
