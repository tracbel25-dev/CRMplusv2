...
alter table public.products
  ...
  add column if not exists daily_stock_date date;

alter table public.products
  drop constraint if exists products_daily_limit_positive;

alter table public.products
  add constraint products_daily_limit_positive
...
begin
  ...
  loop
    ...
    if v_product.sold_out_until is not null and v_product.sold_out_until >= current_date then
      raise exception '% está esgotado por hoje.', v_product.name;
    end if;

    if v_product.daily_limit is not null and v_product.daily_limit > 0 and v_product.daily_stock_date is distinct from current_date then
      update public.products
      set stock = daily_limit,
          daily_stock_date = current_date,
          updated_at = now()
      where tenant_key = v_tenant and id = v_product.id
      returning * into v_product;
    end if;

    if v_product.stock_controlled and v_product.stock < v_stock.quantity then
      raise exception 'Estoque insuficiente: %.', v_product.name;
    end if;
    if v_product.stock_controlled then v_any_reserved := true; end if;
  end loop;
