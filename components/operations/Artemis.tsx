...
export function Artemis({ w, page, recordId = '', embedded = false, rushMode = false, publicSlug = '' }: { w: Workspace; page: string; recordId?: string; embedded?: boolean; rushMode?: boolean; publicSlug?: string }) {
  ...
  return <>
    {current ? <LeanArtemisOrderDetail w={w} recordId={current.id} /> : <>
      ...
      })}</div>{!d.tables.length && <Empty icon={<UtensilsCrossed size={30} />}>{embedded ? 'Nenhuma mesa foi cadastrada pela gestão.' : 'Cadastre as mesas reais do seu salão para abrir comandas.'}</Empty>}</>}
...
function NewOrder({ w, tableId, onCreated, onClose }: { w: Workspace; tableId: string; onCreated: (id: string) => void; onClose: () => void }) {
  ...
  return <>
    ...
    <RecordForm draftKey={`artemis-order:${tableId || 'avulso'}:${channel}`} key={channel} fields={[
      ...
      ...((operation.fieldVisible('customerPhone') || identityRequired) ? [{ name: 'phone', label: operation.label('customerPhone', 'Telefone'), type: 'tel', required: identityRequired }] : []),
      ...
      if (resolved) setCustomValues(data, resolved.id, Object.fromEntries(operation.preferences.customFields.filter(field => field.group === 'Cliente').map(field => [field.id, custom[field.id] || customValues(data, resolved.id)[field.id] || ''])));
      onCreated(order.id);
    }, 'Pedido registrado.')} />
  </>;
}
