...
import {
  Line, Question, Quote, Survey, blankQuote, customValues, date, decideQuote,
...
export function Research({ w, page, recordId = '' }: { w: Workspace; page: string; recordId?: string }) {
  ...
  const [create, setCreate] = useState(false);
  const [selected, setSelected] = useRecordRoute(recordId, '/athena-pesquisa/pesquisas');
  const [edit, setEdit] = useState(false);
  ...
  const surveyCustom = operation.preferences.customFields.filter(field => field.visible && ['Pesquisa', 'Perguntas', 'Coleta', 'Resultado'].includes(field.group));
  const respondentCustom = operation.preferences.customFields.filter(field => field.visible && field.group === 'Respondente');
  ...
  if (survey && apply) return <Respond w={w} survey={survey} respondentCustom={respondentCustom} onClose={() => setApply(false)} />;

  if (survey) {
    const metadata = customValues(d, survey.id);
    return <>
  ...
  if (page === 'respostas' || page === 'resultados') return <>
    ...
    {page === 'resultados'
      ...
      : responses.map(response => { const current = d.surveys.find(item => item.id === response.surveyId); const follow = customValues(d, response.id); return <Section key={response.id} title={current?.title || 'Pesquisa'} action={<small className="op-muted">{date(response.at, true)}</small>}>{Object.entries(response.answers).map(([id, value]) => <div className="op-answer" key={id}><span>{current?.questions.find(question => question.id === id)?.title}</span><strong>{value || 'Sem resposta'}</strong></div>)}{respondentCustom.length > 0 && <div className="op-detail-pairs">{respondentCustom.map(field => <div key={field.id}><span>{field.label}</span><strong>{follow[field.id] || 'Não informado'}</strong></div>)}</div>}{response.contact && <div className="op-callout"><strong>Contato autorizado: {response.contact}</strong><p>{follow.owner ? `${follow.done === 'sim' ? 'Providência concluída' : 'Retorno pendente'} · ${follow.owner}${follow.due ? ` · prazo ${date(follow.due)}` : ''}` : 'Sem responsável definido.'}</p><Button variant="secondary" onClick={() => setFollowUp(response.id)}>{follow.owner ? 'Atualizar providência' : 'Atribuir retorno'}</Button></div>}</Section>; })}
  ...
  return <>
    ...
    {create && <Modal title="Nova pesquisa" onClose={() => setCreate(false)}><SurveyCreateForm w={w} fields={surveyCustom} onClose={() => setCreate(false)} onCreated={id => { setSelected(id); setEdit(true); }} /></Modal>}
  </>;
}

function SurveyCreateForm({ w, fields, onClose, onCreated }: { w: Workspace; fields: ReturnType<typeof useOperationPreferences>['preferences']['customFields']; onClose: () => void; onCreated: (id: string) => void }) {
  const operation = useOperationPreferences('athena-pesquisa');
  return <RecordForm draftKey="athena-survey:new" fields={[{ name: 'title', label: operation.label('surveyTitle', 'Nome da pesquisa'), required: true, wide: true }, ...(operation.fieldVisible('surveyDescription') ? [{ name: 'description', label: operation.label('surveyDescription', 'Mensagem para quem vai responder'), type: 'textarea', wide: true }] : []), ...fields.map(field => ({ name: `custom__${field.id}`, label: field.label, wide: true }))]} onClose={onClose} submit="Criar e editar perguntas" onSave={form => w.mutate(data => { const next: Survey = { id: uid(), title: form.title, description: form.description || '', status: 'Rascunho', questions: [], createdAt: now() }; data.surveys.push(next); setCustomValues(data, next.id, Object.fromEntries(fields.map(field => [field.id, form[`custom__${field.id}`] || '']))); onCreated(next.id); })} />;
}
...
function SurveyEditor({ w, survey, customFields, onSave, onClose }: { w: Workspace; survey: Survey; customFields: ReturnType<typeof useOperationPreferences>['preferences']['customFields']; onSave: (value: Survey) => Promise<boolean>; onClose: () => void }) {
  const operation = useOperationPreferences('athena-pesquisa');
  const [draft, setDraft] = useState(() => structuredClone(survey));
  const [metadata, setMetadata] = useState<Record<string, string>>(() => customValues(w.data, survey.id));
  const [error, setError] = useState('');
  const [busy, setBusy] = useState(false);
  ...
  return <form onSubmit={async submit => { submit.preventDefault(); try { validateSurvey(draft); setBusy(true); const ok = await onSave(draft); if (ok) { await w.mutate(data => setCustomValues(data, survey.id, metadata), 'Pesquisa salva.'); onClose(); } } catch (reason) { setError((reason as Error).message); } finally { setBusy(false); } }}>
    ...
    {customFields.length > 0 && <div className="op-fields">{customFields.map(field => <label className="op-field" key={field.id}><span>{field.label}</span><input value={metadata[field.id] || ''} onChange={change => setMetadata({ ...metadata, [field.id]: change.target.value })} /></label>)}</div>}
...
function Respond({ w, survey, respondentCustom, onClose }: { w: Workspace; survey: Survey; respondentCustom: ReturnType<typeof useOperationPreferences>['preferences']['customFields']; onClose: () => void }) {
  const operation = useOperationPreferences('athena-pesquisa');
  ...
  const reset = () => { setAnswers({}); setContact(''); setCustom({}); setSent(false); w.setError(''); };
  ...
  return <div className="research-standalone"><div className="op-form-footer"><span className="op-muted">Modo de coleta · administração protegida</span><Button variant="secondary" onClick={onClose}>Sair da coleta</Button></div><form onSubmit={async submit => {
    ...
    setBusy(true);
    const ok = await w.mutate(data => { const current = data.surveys.find(item => item.id === survey.id); if (!current || current.status !== 'Ativa') throw new Error('Esta pesquisa não está mais aceitando respostas.'); const id = uid(); data.responses.push({ id, surveyId: survey.id, at: now(), answers: structuredClone(answers), contact: contact.trim() }); setCustomValues(data, id, custom); }, 'Resposta registrada.');
    ...
    {respondentCustom.length > 0 && <div className="op-fields">{respondentCustom.map(field => <label className="op-field" key={field.id}><span>{field.label}</span><input value={custom[field.id] || ''} onChange={change => setCustom({ ...custom, [field.id]: change.target.value })} /></label>)}</div>}
