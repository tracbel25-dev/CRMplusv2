...
export function Zeus({ w, page, recordId = '' }: { w: Workspace; page: string; recordId?: string }) {
  ...
  const jobList = (list: Job[]) => list.length
    ? <div className="op-job-list">{list.map(job => {
      ...
      return <button key={job.id} className="op-job" onClick={() => setSelected(job.id)}>
        ...
        <div className="op-job-state"><Badge tone={['Aguardando aprovação', 'Aguardando peça', 'Pausado'].includes(job.status) ? 'warning' : ''}>{job.status}</Badge>{job.due && <small className={new Date(job.due) < new Date() && activeJob(job) ? 'op-overdue' : ''}>{new Date(job.due) < new Date() && activeJob(job) ? 'Prazo vencido · ' : ''}{date(job.due, true)}</small>}<ArrowRight size={18} /></div>
      </button>;
    })}</div>
  ...
  return <>
    {chosen ? <JobDetail w={w} job={chosen} onBack={() => setSelected('')} /> : <>
      {page === 'inicio' && <>
        ...
        {s.budgetEnabled && operation.actionVisible('budget') && d.jobs.some(job => job.status === 'Aguardando aprovação') && <Section title="Aguardando aprovação" className="zeus-pending">{jobList(jobs.filter(job => job.status === 'Aguardando aprovação'))}</Section>}
        {d.jobs.some(job => ['Aguardando peça', 'Pausado'].includes(job.status)) && <Section title="Parados / dependências">{jobList(jobs.filter(job => ['Aguardando peça', 'Pausado'].includes(job.status)))}</Section>}
        <Section title="Em trabalho" action={<span className="op-muted">{d.jobs.filter(job => activeJob(job) && !['Aguardando aprovação', 'Aguardando peça', 'Pausado'].includes(job.status)).length} atendimentos</span>}>{jobList(jobs.filter(job => activeJob(job) && !['Aguardando aprovação', 'Aguardando peça', 'Pausado'].includes(job.status)))}</Section>
...
function customFieldDefs(operation: ReturnType<typeof useOperationPreferences>, groups: string[], values: Record<string, string> = {}) {
...
function JobForm({ w, onClose, onCreated }: { w: Workspace; onClose: () => void; onCreated: (id: string) => void }) {
  ...
  return <>
    ...
    {!asset && !newRecord && <><div className="op-picker-results">{w.data.assets.filter(item => matches(search, item.identifier, w.data.customers.find(customer => customer.id === item.customerId)?.name)).map(item => <button className="op-row" key={item.id} onClick={() => { setAssetId(item.id); setCustomerId(item.customerId); }}><strong>{item.identifier}</strong><span>{item.model} · {w.data.customers.find(customer => customer.id === item.customerId)?.name}</span><ArrowRight size={16} /></button>)}</div><Button variant="secondary" onClick={() => setNewRecord(true)}><Plus size={17} />Cadastrar novo neste atendimento</Button></>}
    {(asset || newRecord) && <><div className="op-callout">{asset ? <><strong>{asset.identifier} · {asset.model}</strong><span>{w.data.customers.find(customer => customer.id === asset.customerId)?.name}</span><button className="op-text-link" onClick={() => { setAssetId(''); setNewRecord(false); }}>Trocar identificação</button></> : <label className="op-field"><span>Cliente</span><select value={customerId} onChange={change => setCustomerId(change.target.value)}><option value="">Cadastrar novo cliente</option>{customerOptions(w.data).map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>}</div>
      <RecordForm draftKey="zeus-job:new" key={assetId + customerId} fields={[
        ...(newRecord ? [...(!customerId ? [{ name: 'customerName', label: operation.label('customerName', 'Nome do cliente'), required: true }, ...(operation.fieldVisible('customerPhone') ? [{ name: 'phone', label: operation.label('customerPhone', 'Telefone'), type: 'tel' }] : [])] : []), { name: 'identifier', label: s.identifierLabel, required: true, value: search }, { name: 'model', label: s.assetLabel, required: true }, ...(operation.fieldVisible('year') ? [{ name: 'year', label: operation.label('year', 'Ano') }] : []), ...(operation.fieldVisible('meter') ? [{ name: 'meter', label: s.meterLabel }] : [])] : []),
      ...
      ]} submit="Abrir atendimento" onClose={onClose} onSave={form => w.mutate(data => {
        let cid = customerId; let aid = assetId;
        if (newRecord) {
          if (!cid) { cid = uid(); data.customers.push({ id: cid, name: form.customerName.trim(), phone: form.phone || '', email: '', notes: '' }); }
          if (data.assets.some(item => item.identifier.replace(/\W/g, '').toUpperCase() === form.identifier.replace(/\W/g, '').toUpperCase())) throw new Error('Esta identificação já existe. Selecione o cadastro.');
          aid = uid(); data.assets.push({ id: aid, customerId: cid, identifier: form.identifier.trim().toUpperCase(), model: form.model, year: form.year || '', meter: form.meter || '' });
        }
        ...
        const customerValues = Object.fromEntries(operation.preferences.customFields.filter(field => field.group === 'Cliente').map(field => [field.id, values[field.id] || '']));
        const assetValues = Object.fromEntries(operation.preferences.customFields.filter(field => field.group === 'Veículo / equipamento').map(field => [field.id, values[field.id] || '']));
        const jobValues = Object.fromEntries(operation.preferences.customFields.filter(field => field.group === 'Atendimento').map(field => [field.id, values[field.id] || '']));
        setCustomValues(data, cid, customerValues); setCustomValues(data, aid, assetValues); setCustomValues(data, id, jobValues); onCreated(id);
      }, 'Atendimento aberto.')} />
      {asset && <Section title="Últimos atendimentos">{w.data.jobs.filter(job => job.assetId === assetId).slice(-3).reverse().map(job => <div className="op-row" key={job.id}><strong>OS {job.number}</strong><span>{date(job.createdAt)} · {job.type}</span><Badge>{job.status}</Badge></div>)}{!w.data.jobs.some(job => job.assetId === assetId) && <p className="op-muted">Primeiro atendimento deste cadastro.</p>}</Section>}
    </>}
  </>;
}
...
function AppointmentForm({ w, appointment, onClose }: { w: Workspace; appointment?: Appointment; onClose: () => void }) {
  ...
  const asset = w.data.assets.find(item => item.id === assetId);
  return <><SearchBox value={search} onChange={setSearch} placeholder={`Buscar ${w.data.settings.identifierLabel.toLowerCase()} ou cliente`} />{!asset && !newRecord && <><div className="op-picker-results">{w.data.assets.filter(item => matches(search, item.identifier, w.data.customers.find(customer => customer.id === item.customerId)?.name)).map(item => <button type="button" className="op-row" key={item.id} onClick={() => { setAssetId(item.id); setCustomerId(item.customerId); }}><strong>{item.identifier}</strong><span>{item.model} · {w.data.customers.find(customer => customer.id === item.customerId)?.name}</span><ArrowRight size={16} /></button>)}</div><Button variant="secondary" onClick={() => setNewRecord(true)}><Plus size={16} />Cadastrar cliente e {w.data.settings.assetLabel.toLowerCase()} aqui</Button></>}
    {(asset || newRecord) && <><div className="op-callout">{asset ? <><strong>{asset.identifier} · {asset.model}</strong><span>{w.data.customers.find(customer => customer.id === asset.customerId)?.name}</span>{!appointment && <button type="button" className="op-text-link" onClick={() => { setAssetId(''); setNewRecord(false); }}>Trocar</button>}</> : <label className="op-field"><span>Cliente</span><select value={customerId} onChange={change => setCustomerId(change.target.value)}><option value="">Cadastrar novo cliente</option>{customerOptions(w.data).map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>}</div>
      <RecordForm draftKey={`zeus-appointment:${appointment?.id || 'new'}`} fields={[
        ...(newRecord ? [...(!customerId ? [{ name: 'customerName', label: operation.label('customerName', 'Nome do cliente'), required: true }, ...(operation.fieldVisible('customerPhone') ? [{ name: 'phone', label: operation.label('customerPhone', 'Telefone'), type: 'tel' }] : [])] : []), { name: 'identifier', label: w.data.settings.identifierLabel, required: true, value: search }, { name: 'model', label: w.data.settings.assetLabel, required: true }, ...(operation.fieldVisible('year') ? [{ name: 'year', label: operation.label('year', 'Ano') }] : []), ...(operation.fieldVisible('meter') ? [{ name: 'meter', label: w.data.settings.meterLabel }] : [])] : []),
        ...
        if (newRecord) {
          if (!cid) { cid = uid(); data.customers.push({ id: cid, name: form.customerName.trim(), phone: form.phone || '', email: '', notes: '' }); }
          if (data.assets.some(item => item.identifier.replace(/\W/g, '').toUpperCase() === form.identifier.replace(/\W/g, '').toUpperCase())) throw new Error('Esta identificação já está cadastrada. Selecione-a na busca.');
          aid = uid(); data.assets.push({ id: aid, customerId: cid, identifier: form.identifier.trim().toUpperCase(), model: form.model, year: form.year || '', meter: form.meter || '' });
        }
        const selectedAsset = data.assets.find(item => item.id === aid);
        if (!selectedAsset) throw new Error('Selecione ou cadastre o veículo.');
        if (data.appointments.some(item => item.id !== appointment?.id && item.at === form.at && item.status === 'Agendado' && form.technician && item.technician === form.technician)) throw new Error('Este responsável já possui um agendamento neste horário.');
        const next: Appointment = { id: appointment?.id || uid(), customerId: selectedAsset.customerId, assetId: selectedAsset.id, at: form.at, type: form.type, technician: form.technician || appointment?.technician || '', notes: form.notes || '', status: 'Agendado' };
        const index = data.appointments.findIndex(item => item.id === next.id);
        if (index < 0) data.appointments.push(next); else { if (data.appointments[index].status !== 'Agendado') throw new Error('Este agendamento já foi iniciado ou cancelado.'); data.appointments[index] = next; }
      })}>{appointment && <Button variant="danger" onClick={async () => { if (await w.mutate(data => { const current = data.appointments.find(item => item.id === appointment.id)!; if (current.status !== 'Agendado') throw new Error('Este agendamento não está disponível.'); current.status = 'Cancelado'; }, 'Agendamento cancelado.')) onClose(); }}>Cancelar agendamento</Button>}</RecordForm>
    </>}
  </>;
}
...
function AssetForm({ w, customerId, onClose }: { w: Workspace; customerId: string; onClose: () => void }) {
  const operation = useOperationPreferences('zeus');
  const s = w.data.settings;
  const custom = operation.preferences.customFields.filter(field => field.visible && field.group === 'Veículo / equipamento');
  return <RecordForm draftKey={`zeus-asset:${customerId}:new`} fields={[{ name: 'identifier', label: s.identifierLabel, required: true }, { name: 'model', label: s.assetLabel, required: true }, ...(operation.fieldVisible('year') ? [{ name: 'year', label: operation.label('year', 'Ano') }] : []), ...(operation.fieldVisible('meter') ? [{ name: 'meter', label: s.meterLabel }] : []), ...custom.map(field => ({ name: `custom__${field.id}`, label: field.label }))]} onClose={onClose} onSave={form => w.mutate(data => { if (data.assets.some(asset => asset.identifier.replace(/\W/g, '').toUpperCase() === form.identifier.replace(/\W/g, '').toUpperCase())) throw new Error('Esta identificação já está cadastrada.'); const id = uid(); data.assets.push({ id, customerId, identifier: form.identifier, model: form.model, year: form.year || '', meter: form.meter || '' } as Asset); setCustomValues(data, id, Object.fromEntries(custom.map(field => [field.id, form[`custom__${field.id}`] || '']))); })} />;
}
...
function JobDetail({ w, job, onBack }: { w: Workspace; job: Job; onBack: () => void }) {
  ...
  const nextStage = flow[flow.indexOf(job.stage) + 1];
  const quoteStatus = effectiveQuoteStatus(job.quote);
  const blocking = job.stage === 'Diagnóstico' && !job.diagnosis.trim() ? { label: 'Registrar diagnóstico', tab: 'Diagnóstico' }
    : job.stage === 'Orçamento' && quoteStatus !== 'Aprovado' ? { label: quoteStatus === 'Expirado' ? 'Revisar orçamento vencido' : 'Resolver aprovação do orçamento', tab: 'Orçamento' }
      : job.stage === 'Execução' && job.tasks.some(item => !item.done) ? { label: `Concluir ${job.tasks.filter(item => !item.done).length} serviço(s) pendente(s)`, tab: 'Execução' } : null;
  const nextLabel = blocking?.label || (job.stage === 'Entrega' ? 'Registrar entrega e encerrar' : `Avançar para ${nextStage || 'próxima etapa'}`);
  const tabs = ['Ficha', ...(s.diagnosisEnabled && operation.actionVisible('diagnosis') ? ['Diagnóstico'] : []), ...(s.budgetEnabled && operation.actionVisible('budget') ? ['Orçamento'] : []), 'Execução', ...(operation.actionVisible('attachments') ? ['Anexos'] : []), 'Fechamento', 'Linha do tempo'];
  ...
  const advance = async () => {
    if (blocking) { setTab(blocking.tab); return; }
    if (job.stage === 'Entrega') { setFinish(true); return; }
    const ok = await w.mutate(data => advanceJob(data, job.id, job.stage), 'Etapa atualizada.');
    if (!ok) return;
    const destination = nextStage === 'Diagnóstico' ? 'Diagnóstico' : nextStage === 'Orçamento' ? 'Orçamento' : nextStage === 'Execução' ? 'Execução' : ['Conferência', 'Entrega'].includes(nextStage || '') ? 'Fechamento' : 'Ficha';
    setTab(destination);
  };

  return <>
    <Button variant="text" onClick={onBack}><ArrowLeft size={17} />Voltar aos atendimentos</Button>
    <Title eyebrow={`Ordem de serviço ${String(job.number).padStart(4, '0')} · ${job.type}`} title={asset.identifier} action={operation.actionVisible('report') ? <Button variant="secondary" onClick={() => setReport(true)}><FileDown size={16} />Relatório</Button> : undefined}>{customer.name} · {asset.model}{job.technician && ` · ${job.technician}`}</Title>
    <WorkflowControl label="Etapa da ordem de serviço" steps={flow} current={job.stage} status={job.status} nextLabel={activeJob(job) ? nextLabel : undefined} onNext={activeJob(job) ? () => { void advance(); } : undefined} actions={activeJob(job) && operation.actionVisible('manualStatus') && <select aria-label="Situação do atendimento" value={job.status} disabled={job.status === 'Aguardando aprovação'} onChange={change => patch(current => { current.status = change.target.value; current.events.push(event(`Situação: ${current.status}`)); })}>{Array.from(new Set([job.status, 'Em andamento', 'Aguardando peça', 'Pausado'])).map(value => <option key={value}>{value}</option>)}</select>} />
    {blocking && <p className="op-callout">Antes de avançar: {blocking.label}. O botão acima leva diretamente ao ponto que precisa ser resolvido.</p>}
    ...
    {tab === 'Anexos' && <Section title="Fotos do atendimento" action={activeJob(job) && <label className="op-button secondary">Adicionar foto<input hidden type="file" accept="image/jpeg,image/png,image/webp" onChange={async input => { const file = input.target.files?.[0]; if (!file) return; if (file.size > 750000) { w.setError('Escolha uma foto de até 750 KB nesta versão local.'); return; } if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) { w.setError('Use uma imagem JPEG, PNG ou WebP.'); return; } const reader = new FileReader(); reader.onload = () => patch(current => { current.attachments.push({ id: uid(), name: file.name, data: String(reader.result) }); current.events.push(event(`Foto anexada: ${file.name}`)); }); reader.readAsDataURL(file); input.target.value = ''; }} /></label>}><div className="op-photos">{job.attachments.map(photo => <figure key={photo.id}><img src={photo.data} alt={photo.name} /><figcaption>{photo.name}</figcaption>{activeJob(job) && <button className="op-icon" aria-label={`Remover ${photo.name}`} onClick={() => patch(current => { current.attachments = current.attachments.filter(item => item.id !== photo.id); })}><X size={16} /></button>}</figure>)}</div>{!job.attachments.length && <Empty>Adicione fotos como evidência do atendimento.</Empty>}</Section>}
    {tab === 'Fechamento' && <Section title="Conferência e entrega"><div className="op-detail-pairs"><div><span>Serviços concluídos</span><strong>{job.tasks.filter(item => item.done).length} de {job.tasks.length}</strong></div><div><span>Situação atual</span><strong>{job.status}</strong></div><div><span>Etapa</span><strong>{job.stage}</strong></div><div><span>Peças registradas</span><strong>{job.quote.lines.filter(line => line.kind === 'Peça').length}</strong></div></div>{activeJob(job) && <div className="op-actions"><Button variant="secondary" onClick={() => setFinalNote(true)}>Registrar observação final</Button>{job.stage === 'Entrega' && <Button onClick={() => setFinish(true)}>Registrar entrega e encerrar</Button>}</div>}</Section>}
