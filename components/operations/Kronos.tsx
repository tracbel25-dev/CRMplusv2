...
import {
  ...
  Plus, Sparkles, Target, ThermometerSun, TriangleAlert, Users
} from 'lucide-react';
import {
...
import {
  getKronosDealMeta, getKronosVisits, isClosedDeal, kronosSignal, newKronosVisit,
  setKronosDealMeta, updateKronosVisit, upsertKronosVisit, type KronosDealMeta,
  type KronosQuoteStatus, type KronosTemperature, type KronosVisit
...
import {
  ...
  Timeline, Title, customerOptions
} from './ui';
...
export function Kronos({ w, page, recordId = '' }: { w: Workspace; page: string; recordId?: string }) {
  ...
  const needsAttention = active
    .map(item => ({ deal: item, signal: signalFor(item) }))
    .filter(item => item.signal.label === 'Atenção')
    .sort((a, b) => a.signal.score - b.signal.score);
  const hot = active
    .map(item => ({ deal: item, signal: signalFor(item), meta: getKronosDealMeta(d, item.id) }))
    .filter(item => item.signal.label === 'Forte' || item.meta.temperature === 'Quente')
  ...
  if (deal) {
    ...
    return <>
      ...
      <WorkflowControl
        label="Momento da negociação"
        steps={workflowSteps}
        current={deal.stage}
        status={deal.stage}
        actions={activeDeal && nextStage && operation.actionVisible('quickAdvance') ? <Button variant="secondary" onClick={() => { void w.mutate(data => {
      ...
      <Section title="Visitas, reuniões e ligações">
        <div className="op-list">{dealVisits.map(item => <div className="op-row" key={item.id}><div className="op-grow"><strong>{item.kind} · {dayLabel(item.at)}</strong><small>{item.objective || item.offering || 'Sem objetivo informado'}{item.result ? ` · ${item.result}` : ''}</small></div><Badge>{item.status}</Badge></div>)}{!dealVisits.length && <Empty>Nenhum compromisso do cronograma está ligado a esta oportunidade.</Empty>}</div>
      </Section>
      ...
      {task && <Modal title="Agendar próxima ação" onClose={() => setTask(false)}><RecordForm draftKey={`kronos-task:${deal.id}`} fields={[{ name: 'title', label: 'O que precisa ser feito?', required: true, wide: true }, { name: 'due', label: 'Quando?', type: 'datetime-local', required: true }]} onClose={() => setTask(false)} onSave={form => w.mutate(data => {
        data.tasks.push({ id: uid(), dealId: deal.id, title: form.title, due: form.due, done: false });
        const current = data.deals.find(item => item.id === deal.id)!;
        current.events.push(event(`Próxima ação: ${form.title} · ${date(form.due, true)}`));
        syncDealNextActivity(data, deal.id);
      })} /></Modal>}
      ...
      {contact && <Modal title="Registrar contato" onClose={() => setContact(false)}><RecordForm draftKey={`kronos-contact:${deal.id}`} fields={[
        ...
        { name: 'stage', label: 'Momento após o contato', options: columns.map(value => ({ value, label: value })), value: deal.stage },
      ...
      {result && <Modal title={result === 'Ganha' ? 'Confirmar venda realizada' : 'Registrar perda da negociação'} onClose={() => setResult('')}><RecordForm fields={[
        ...
        ...(pendingFor(deal.id).length ? [{ name: 'pending', label: `${pendingFor(deal.id).length} ação(ões) ainda pendente(s)`, required: true, options: [{ value: 'close', label: 'Encerrar junto com a negociação' }, { value: 'keep', label: 'Manter para acompanhamento posterior' }] }] : [])
      ]} onClose={() => setResult('')} submit={result === 'Ganha' ? 'Confirmar venda' : 'Confirmar perda'} onSave={form => w.mutate(data => {
        const current = data.deals.find(item => item.id === deal.id)!;
        if (isClosedDeal(current)) throw new Error('Esta negociação já foi encerrada.');
        ...
        syncDealNextActivity(data, deal.id);
        current.events.push(event(`${result === 'Ganha' ? 'Venda confirmada' : 'Negociação perdida'}: ${form.note}`));
      })} /></Modal>}
    </>;
  }

  const title = page === 'inicio' ? 'O que merece sua atenção agora?'
    : page === 'cronograma' ? 'Cronograma comercial'
      : page === 'atividades' ? 'Acompanhamento'
        : page === 'clientes' ? 'Carteira de clientes'
          : page === 'historico' ? 'Negociações encerradas'
            : 'Oportunidades';

  return <>
    {page !== 'clientes' && <Title eyebrow="Kronos / Operação comercial" title={title} action={<div className="op-actions"><Button variant="secondary" onClick={() => setLeadCreate(true)}><Users size={17} />Novo lead</Button>{page === 'cronograma' ? <Button onClick={() => setVisitCreate(true)}><CalendarCheck2 size={17} />Novo compromisso</Button> : <Button onClick={() => setCreate(true)}><Plus size={18} />Nova oportunidade</Button>}</div>}>
      {page === 'inicio' ? 'O funil organiza os dados por baixo. Aqui aparecem somente ações, riscos e oportunidades que ajudam a vender.' : undefined}
    </Title>}

    {page === 'inicio' && <>
      <div className="kronos-decision-summary">
        <div><Target size={20} /><span>Em negociação</span><strong>{active.length}</strong><small>{money(totalOpen)} em potencial registrado</small></div>
        <div><TriangleAlert size={20} /><span>Precisam de atenção</span><strong>{needsAttention.length}</strong><small>Sem próxima ação, atrasadas ou paradas</small></div>
        <div><Flame size={20} /><span>Sinais fortes</span><strong>{hot.length}</strong><small>Negociações com sinais de avanço</small></div>
        <div><CalendarCheck2 size={20} /><span>Próximos compromissos</span><strong>{upcomingVisits.length}</strong><small>Visitas, reuniões e ligações planejadas</small></div>
      </div>

      <div className="op-split kronos-home-grid">
        <Section title="Sua próxima ação">
          <div className="kronos-action-list">{active.map(item => ({ deal: item, task: nextTaskFor(item.id) })).filter(item => item.task).sort((a, b) => a.task!.due.localeCompare(b.task!.due)).slice(0, 7).map(({ deal: item, task: next }) => <button key={item.id} onClick={() => setSelected(item.id)}><time className={new Date(next!.due) < new Date() ? 'op-overdue' : ''}>{new Date(next!.due) < new Date() ? 'Atrasado' : dayLabel(next!.due)}</time><div><strong>{customer(item.customerId)?.name}</strong><span>{next!.title}</span><small>{item.title}</small></div><ArrowRight size={18} /></button>)}{!active.some(item => nextTaskFor(item.id)) && <Empty>Defina próximos passos nas oportunidades para o Kronos montar sua rotina comercial.</Empty>}</div>
        </Section>

        <Section title="Não deixe esfriar">
          <div className="kronos-attention-list">{needsAttention.slice(0, 6).map(({ deal: item, signal }) => <button key={item.id} onClick={() => setSelected(item.id)}><TriangleAlert size={18} /><div><strong>{customer(item.customerId)?.name}</strong><span>{item.title}</span><small>{signal.reasons.join(' · ')}</small></div><Badge>{signal.score}/100</Badge></button>)}{!needsAttention.length && <Empty>Nenhuma oportunidade crítica agora.</Empty>}</div>
        </Section>
      </div>

      <Section title="Negociações com sinais de avanço">
        <div className="kronos-opportunity-grid">{hot.slice(0, 6).map(({ deal: item, signal, meta }) => <button key={item.id} onClick={() => setSelected(item.id)}><div className="kronos-card-top"><span>{meta.motion || 'Origem não definida'}</span><Badge>{signal.label}</Badge></div><strong>{customer(item.customerId)?.name}</strong><h3>{item.title}</h3><div className="kronos-card-meta"><span>{money(item.value)}</span><span>{meta.temperature ? `Vendedor: ${meta.temperature}` : 'Sem percepção informada'}</span></div><small>{signal.reasons.join(' · ')}</small></button>)}{!hot.length && <Empty>À medida que contatos, retornos e propostas forem registrados, o Kronos destacará aqui as negociações com melhores sinais.</Empty>}</div>
      </Section>
    </>}
    ...
    {page === 'clientes' && <CustomerManager w={w} title="Carteira" onOpen={customerRecord => <>
      <Section title="Oportunidades">{d.deals.filter(item => item.customerId === customerRecord.id).map(item => <button className="op-row" key={item.id} onClick={() => setSelected(item.id)}><strong className="op-grow">{item.title}</strong><Badge>{item.stage}</Badge><b>{money(item.value)}</b></button>)}{!d.deals.some(item => item.customerId === customerRecord.id) && <Empty>Nenhuma oportunidade registrada.</Empty>}</Section>
      <Section title="Cronograma">{visits.filter(item => item.customerId === customerRecord.id).map(item => <div className="op-row" key={item.id}><div className="op-grow"><strong>{item.kind}</strong><small>{date(item.at, true)} · {item.objective || item.offering}</small></div><Badge>{item.status}</Badge></div>)}{!visits.some(item => item.customerId === customerRecord.id) && <Empty>Nenhum compromisso registrado.</Empty>}</Section>
    </>} />}

    {page === 'oportunidades' && <>
      <div className="op-toolbar"><SearchBox value={query} onChange={setQuery} placeholder="Buscar oportunidade, cliente ou origem" /><span className="op-muted">{active.length} abertas · {money(totalOpen)} em potencial</span></div>
      <div className="kronos-opportunity-list">{active.sort((a, b) => signalFor(a).score - signalFor(b).score).map(item => {
        const meta = getKronosDealMeta(d, item.id);
        const signal = signalFor(item);
        const next = nextTaskFor(item.id);
        return <button key={item.id} onClick={() => setSelected(item.id)}><div className="kronos-opportunity-main"><span className="op-kicker">{customer(item.customerId)?.name}</span><h3>{item.title}</h3><small>{meta.motion || 'Origem não definida'}{item.source ? ` · ${item.source}` : ''}</small></div><div><span>Momento</span><strong>{item.stage}</strong></div><div><span>Termômetro</span><strong>{meta.temperature || '—'}</strong><small>Kronos: {signal.label} {signal.score}/100</small></div><div><span>Próxima ação</span><strong className={next && new Date(next.due) < new Date() ? 'op-overdue' : ''}>{next ? dayLabel(next.due) : 'Sem ação'}</strong><small>{next?.title || signal.reasons[0]}</small></div><div className="kronos-value"><strong>{money(item.value)}</strong><ArrowRight size={17} /></div></button>;
      })}{!active.length && <Empty>Crie o primeiro lead ou oportunidade para iniciar a operação comercial.</Empty>}</div>
    </>}

    {create && <Modal title="Nova oportunidade" onClose={() => setCreate(false)}>{d.customers.length ? <DealForm w={w} onClose={() => setCreate(false)} onCreated={id => setSelected(id)} /> : <Empty>Cadastre um lead primeiro. O Kronos cria a pessoa/empresa e a oportunidade no mesmo fluxo.</Empty>}</Modal>}
...
function VisitForm({ w, onClose }: { w: Workspace; onClose: () => void }) {
  ...
  return <RecordForm draftKey="kronos-visit:new" fields={[
    { name: 'customerId', label: 'Cliente / lead', required: true, options: customerOptions(w.data) },
  ...
  ]} onClose={onClose} submit="Adicionar ao cronograma" onSave={form => w.mutate(data => {
    if (!data.customers.some(item => item.id === form.customerId)) throw new Error('Selecione um cliente ou lead.');
    if (form.dealId && !data.deals.some(item => item.id === form.dealId && item.customerId === form.customerId)) throw new Error('A oportunidade selecionada pertence a outro cliente.');
    const quoteValue = form.quoteValue ? cents(form.quoteValue) : 0;
    const visit = newKronosVisit({ customerId: form.customerId, dealId: form.dealId || '', kind: form.kind as KronosVisit['kind'], at: form.at, offering: form.offering || '', objective: form.objective || '', quoteStatus: (form.quoteStatus || 'Sem cotação') as KronosQuoteStatus, quoteValue, notes: form.notes || '' });
    upsertKronosVisit(data, visit);
    if (visit.dealId) {
      const deal = data.deals.find(item => item.id === visit.dealId)!;
      deal.events.push(event(`${visit.kind} agendada para ${date(visit.at, true)}${visit.objective ? ` · Objetivo: ${visit.objective}` : ''}`));
      setKronosDealMeta(data, deal.id, { quoteStatus: visit.quoteStatus, quoteValue: visit.quoteValue });
    }
...
function VisitResultForm({ w, visit, onClose }: { w: Workspace; visit: KronosVisit; onClose: () => void }) {
  ...
  return <RecordForm draftKey={`kronos-visit-result:${visit.id}`} fields={[
    ...
    ...(visit.dealId ? [{ name: 'temperature', label: 'Percepção da oportunidade', options: temperatureOptions, value: meta?.temperature }] : []),
    ...
    if (visit.dealId) {
      const deal = data.deals.find(item => item.id === visit.dealId)!;
      setKronosDealMeta(data, deal.id, { temperature: (form.temperature || meta?.temperature || '') as KronosTemperature, lastContactAt: now(), lastOutcome: form.result });
      if (form.followUp && form.followUpAt) data.tasks.push({ id: uid(), dealId: deal.id, title: form.followUp, due: form.followUpAt, done: false });
      syncDealNextActivity(data, deal.id);
      deal.events.push(event(`${visit.kind} realizada: ${form.result}${form.followUp ? ` · Próximo passo: ${form.followUp}` : ''}`));
    }
...
function LeadForm({ w, onClose, onCreated }: { w: Workspace; onClose: () => void; onCreated: (id: string) => void }) {
  return <RecordForm draftKey="kronos-lead:new" fields={[
    { name: 'name', label: 'Nome ou empresa', required: true, wide: true },
    { name: 'phone', label: 'Telefone', type: 'tel' },
    { name: 'email', label: 'E-mail', type: 'email' },
    { name: 'title', label: 'O que podemos oferecer?', required: true, wide: true },
    { name: 'motion', label: 'Como essa oportunidade nasceu?', required: true, options: motionOptions },
    { name: 'source', label: 'Origem / canal', wide: true, hint: 'Ex.: indicação, prospecção, site, WhatsApp, evento.' },
    { name: 'temperature', label: 'Percepção inicial', options: temperatureOptions },
    { name: 'value', label: 'Potencial estimado (R$)', type: 'number', min: 0, step: 0.01 },
    { name: 'notes', label: 'Contexto inicial', type: 'textarea', wide: true },
    { name: 'nextAction', label: 'Primeiro próximo passo', wide: true },
    { name: 'due', label: 'Quando?', type: 'datetime-local' }
  ]} onClose={onClose} submit="Criar lead" onSave={form => w.mutate(data => {
    if ((form.nextAction && !form.due) || (!form.nextAction && form.due)) throw new Error('Informe o próximo passo e a data juntos.');
    const customerId = uid();
    data.customers.push({ id: customerId, name: form.name.trim(), phone: form.phone || '', email: form.email || '', notes: '' });
    const stages = data.settings.salesStages?.length >= 2 ? data.settings.salesStages : fallbackColumns;
    const dealId = uid();
    data.deals.push({ id: dealId, number: nextNumber(data.deals), title: form.title, customerId, value: form.value ? cents(form.value) : 0, stage: stages[0], source: form.source || '', nextAction: '', due: '', notes: form.notes || '', lostReason: '', events: [event(`Lead criado · ${form.motion}${form.source ? ` · Origem: ${form.source}` : ''}`)], createdAt: now() });
    ...
    syncDealNextActivity(data, dealId);
    onCreated(dealId);
  }, 'Lead criado.')} />;
}
...
function DealForm({ w, deal, onClose, onCreated }: { w: Workspace; deal?: Deal; onClose: () => void; onCreated: (id: string) => void }) {
  ...
  return <RecordForm draftKey={`kronos-deal:${deal?.id || 'new'}`} fields={[
    ...
    { name: 'customerId', label: operation.label('customer', 'Cliente / lead'), required: true, options: customerOptions(w.data), value: deal?.customerId },
    ...
    { name: 'temperature', label: 'Percepção do vendedor', options: temperatureOptions, value: meta?.temperature },
    { name: 'quoteStatus', label: 'Situação da cotação', options: quoteOptions, value: meta?.quoteStatus || 'Sem cotação' },
    ...
    if (!data.customers.some(customer => customer.id === form.customerId)) throw new Error('Selecione um cliente ou lead.');
    ...
    if (deal) {
      ...
      if (form.value !== undefined) current.value = form.value ? cents(form.value) : 0;
      ...
      if (form.due !== undefined || form.nextAction !== undefined) {
        if (form.due && form.nextAction) { if (pending) { pending.due = form.due; pending.title = form.nextAction; } else data.tasks.push({ id: uid(), dealId: deal.id, title: form.nextAction, due: form.due, done: false }); }
        else if (pending) pending.done = true;
      }
      syncDealNextActivity(data, deal.id);
      setKronosDealMeta(data, deal.id, { motion: (form.motion || meta?.motion || '') as KronosDealMeta['motion'], temperature: (form.temperature || meta?.temperature || '') as KronosTemperature, quoteStatus: (form.quoteStatus || meta?.quoteStatus || 'Sem cotação') as KronosQuoteStatus, quoteValue: form.quoteValue ? cents(form.quoteValue) : 0 });
      current.events.push(event('Oportunidade atualizada'));
    } else {
      const stages = data.settings.salesStages?.length >= 2 ? data.settings.salesStages : fallbackColumns;
      const record: Deal = { id: recordIdValue, number: nextNumber(data.deals), title: form.title, customerId: form.customerId, value: form.value ? cents(form.value) : 0, source: form.source || '', due: '', nextAction: '', notes: form.notes || '', stage: stages[0], lostReason: '', createdAt: now(), events: [event('Oportunidade criada')] };
      data.deals.push(record);
      setKronosDealMeta(data, record.id, { motion: (form.motion || '') as KronosDealMeta['motion'], temperature: (form.temperature || '') as KronosTemperature, quoteStatus: (form.quoteStatus || 'Sem cotação') as KronosQuoteStatus, quoteValue: form.quoteValue ? cents(form.quoteValue) : 0 });
