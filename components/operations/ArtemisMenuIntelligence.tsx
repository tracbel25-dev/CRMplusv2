...
function VariantEditor({ w, product, suggested, onClose }: { w: Workspace; product: Product; suggested: string[]; onClose: () => void }) {
  ...
  const save = async () => {
    ...
    if (!named.length) { w.setError('Cadastre pelo menos uma opção ou remova todas para deixar o produto sem variações.'); return; }
  ...
  return <div className="artemis-variant-editor">
    ...
    <div className="op-form-footer"><Button variant="secondary" disabled={saving} onClick={onClose}>Cancelar</Button><Button disabled={saving} onClick={() => void save()}>{saving ? 'Salvando…' : 'Salvar opções'}</Button></div>
