...
export function LeanZeusJobDetail({ w, recordId }: { w: Workspace; recordId: string }) {
  ...
  return <>
    ...
    {finish && <Modal title="Entrega e encerramento" onClose={() => setFinish(false)}><div className="op-callout"><strong>Cobrança opcional</strong><span>Você pode encerrar a OS sem cobrar agora ou registrar como o cliente será cobrado.</span></div><RecordForm draftKey={`zeus-finish:${job.id}`} fields={[{ name: 'paymentMethod', label: 'Forma de pagamento', required: true, value: 'Sem cobrança agora', options: [{ value: 'Sem cobrança agora', label: 'Sem cobrança agora' }, { value: 'Na loja', label: 'Na loja' }, { value: 'Débito', label: 'Débito' }, { value: 'Mercado Pago (API)', label: 'Mercado Pago (API)' }] }]} submit="Encerrar atendimento" onClose={() => setFinish(false)} onSave={async form => {
      if (form.paymentMethod === 'Mercado Pago (API)') {
        w.setError('A conta Mercado Pago do cliente precisa estar conectada antes de cobrar pela API. A OS não foi encerrada.');
        return false;
      }
      const ok = await w.mutate(data => {
        const current = data.jobs.find(item => item.id === job.id)!;
