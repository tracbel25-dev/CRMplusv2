  ...
  return <>
    ...
    <div className="zeus-workbench" style={{ marginTop: 18 }}>
      <main className="zeus-now" id="zeus-current-work">
        ...
        {job.stage === 'Execução' && <Section title="Serviços em execução" action={active ? <Button variant="secondary" onClick={() => setEditMode('Execução')}><Plus size={16} />Adicionar tarefa / atualização</Button> : undefined}>{job.tasks.map(task => <label className="op-check-row" key={task.id}><input type="checkbox" checked={!!task.done} disabled={!active || job.stage !== 'Execução'} onChange={change => patch(current => { const line = current.tasks.find(item => item.id === task.id)!; line.done = change.target.checked; current.events.push(event(`${line.done ? 'Concluído' : 'Reaberto'}: ${line.description}`)); })} /><strong>{task.description}</strong><Badge>{task.done ? 'Concluído' : 'Pendente'}</Badge></label>)}{!job.tasks.length && <Empty>Adicione as tarefas reais desta execução.</Empty>}</Section>}
