  ...
  return <>
    ...
    <div className="zeus-workbench" style={{ marginTop: 18 }}>
      <main className="zeus-now" id="zeus-current-work">
        ...
        {job.stage === 'Diagnóstico' && <Section title={label('diagnosis', 'Diagnóstico')} action={active && canEdit ? <Button variant="secondary" onClick={() => setDiagnosisOpen(true)}>{job.diagnosis ? 'Editar diagnóstico' : 'Registrar diagnóstico'}</Button> : undefined}>{job.diagnosis ? <p className="op-prewrap">{job.diagnosis}</p> : <Empty>{canEdit ? 'Registre os achados técnicos. A IA pode sugerir verificações sem preencher o diagnóstico por você.' : 'O diagnóstico ainda não foi registrado.'}</Empty>}</Section>}
