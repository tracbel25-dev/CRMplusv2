...
function DiagnosisEditor({ w, job, assetLabel, canUseAI, onClose }: { w: Workspace; job: Job; assetLabel: string; canUseAI: boolean; onClose: () => void }) {
...
export function LeanZeusJobDetail({ w, recordId }: { w: Workspace; recordId: string }) {
  ...
  const canUseAI = access.hasPermission('zeus', 'ai_use') && zeusViewHasFeature(w.data.settings, 'ai');
  ...
  const identificationPending = job.stage === 'Identificação' && job.status === 'Aguardando checklist';
  const workTitle = job.stage === 'Identificação' ? (identificationPending ? 'Concluir identificação' : 'Identificação concluída')
  ...
  const nowLabel = job.stage === 'Identificação' ? (identificationPending ? 'Concluir checklist da identificação' : nextStage ? `Aguardando iniciar ${nextStage.toLowerCase()}` : 'Identificação concluída')
  ...
  const advance = async () => {
    if (!active) return;
    if (!canAdvance) { w.setError('Seu perfil não possui permissão para avançar etapas da OS.'); return; }
    if (identificationPending) { w.setError('Conclua o checklist de entrada antes de avançar a OS.'); return; }
    if (blockedByDiagnosis) { if (canEdit) setDiagnosisOpen(true); else w.setError('O diagnóstico precisa ser registrado por alguém com permissão de edição.'); return; }
  ...
  const showPrimary = active && canAdvance && job.stage !== 'Orçamento' && !!primaryButtonLabel;
  ...
  const removePhoto = async (photo: Job['attachments'][number]) => {
    if (!canAttach) { w.setError('Seu perfil não possui permissão para remover anexos.'); return; }
  ...
  const toggleExecutionTask = async (taskId: string) => {
    if (!active || job.stage !== 'Execução' || !canEdit) return;
    await patch(current => {
  ...
  return <>
    ...
    <Title eyebrow={`OS ${String(job.number).padStart(4, '0')} · ${job.type}`} title={asset.identifier} action={reportReady ? <Button variant="secondary" onClick={() => setReport(true)}><FileDown size={16} />Relatório</Button> : undefined}>{customer.name} · {asset.model}{job.technician && ` · ${job.technician}`}</Title>
    ...
    <div className="zeus-workbench" style={{ marginTop: 18 }}>
      <main className="zeus-now" id="zeus-current-work">
        <div className="zeus-now-head"><div><span>Trabalho atual</span><strong>{workTitle}</strong><small>{job.stage === 'Orçamento' ? 'Monte, salve e compartilhe quando estiver pronto.' : nowLabel}</small></div></div>
        ...
        {job.stage === 'Identificação' && <Section title="Ficha identificada" action={active && canEdit ? <Button variant="secondary" onClick={() => setEditMode('Ficha')}>Editar ficha</Button> : undefined}><p className="op-prewrap">{job.complaint}</p></Section>}
        {job.stage === 'Diagnóstico' && <Section title={label('diagnosis', 'Diagnóstico')} action={active && canEdit ? <Button variant="secondary" onClick={() => setDiagnosisOpen(true)}>{job.diagnosis ? 'Editar diagnóstico' : 'Registrar diagnóstico'}</Button> : undefined}>{job.diagnosis ? <p className="op-prewrap">{job.diagnosis}</p> : <Empty>{canEdit ? (canUseAI ? 'Registre os achados técnicos. A IA pode sugerir verificações sem preencher o diagnóstico por você.' : 'Registre os achados técnicos do diagnóstico.') : 'O diagnóstico ainda não foi registrado.'}</Empty>}</Section>}
        {job.stage === 'Orçamento' && <Section title="Orçamento desta OS"><ZeusQuotePanel w={w} quote={job.quote} job={job} /></Section>}
        {job.stage === 'Execução' && <Section title="Serviços em execução" action={active && canEdit ? <div className="op-actions">{canUseAI && <ZeusStageAssistant w={w} job={job} />}<Button variant="secondary" onClick={() => setEditMode('Execução')}><Plus size={16} />Adicionar tarefa / atualização</Button></div> : undefined}>{job.tasks.map(task => <button type="button" className="op-check-row" key={task.id} disabled={!active || job.stage !== 'Execução' || !canEdit} aria-pressed={!!task.done} onClick={() => { void toggleExecutionTask(task.id); }} style={{ width: '100%', textAlign: 'left', background: 'transparent', color: 'inherit', borderLeft: 0, borderRight: 0, borderTop: 0 }}><span aria-hidden="true" style={{ width: 26, height: 26, flex: '0 0 26px', display: 'grid', placeItems: 'center', border: '1px solid var(--op-line)', background: task.done ? 'var(--op-accent)' : 'var(--op-paper)', color: task.done ? 'var(--op-on-accent)' : 'transparent' }}><Check size={16} /></span><strong>{task.description}</strong><Badge>{task.done ? 'Concluído' : 'Pendente'}</Badge></button>)}{!job.tasks.length && <Empty>Adicione as tarefas reais desta execução.</Empty>}</Section>}
        {['Conferência', 'Entrega'].includes(job.stage) && <Section title={job.stage === 'Conferência' ? 'Conferência final' : 'Entrega'}><div className="op-detail-pairs"><div><span>Serviços concluídos</span><strong>{job.tasks.filter(item => item.done).length} de {job.tasks.length}</strong></div><div><span>Situação</span><strong>{job.status}</strong></div><div><span>Peças registradas</span><strong>{job.quote.lines.filter(line => line.kind === 'Peça').length}</strong></div><div><span>Orçamento</span><strong>{s.budgetEnabled ? quoteStatus : 'Não utilizado'}</strong></div></div>{active && canEdit && <Button variant="secondary" onClick={() => setFinalNote(true)}>Registrar observação final</Button>}</Section>}
        {!active && <><Section title="Atendimento encerrado"><Badge>{job.status}</Badge><p>O trabalho desta OS foi preservado. Se houver valor a receber, a cobrança pode ser feita diretamente nesta OS.</p></Section>{job.status === 'Encerrado' && zeusViewHasFeature(s, 'payments') && <PostCompletionPayments w={w} app="zeus" page="atendimentos" recordId={job.id} />}</>}
