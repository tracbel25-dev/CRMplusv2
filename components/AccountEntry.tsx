...
export function AccountEntry(){
  ...
  const contractedIds=new Set(access.account.apps.filter(item=>['trialing','active'].includes(item.status)&&(!item.currentPeriodEnd||Date.parse(item.currentPeriodEnd)>Date.now())).map(item=>item.appId));
  ...
  return <>
    ...
    <div className="entry-empty-actions"><Link className="ghost" href="/assinaturas">Gerenciar assinaturas</Link></div><section className="entry-store-more"><div><span className="eyebrow">CRM PLUS Store</span><h2>Quer adicionar outro aplicativo à conta?</h2><p>A contratação acontece pela Store. Depois da ativação, o titular decide quais usuários podem abrir e configurar cada aplicativo.</p></div><Link className="primary" href="/aplicativos">Explorar Store <ArrowRight size={16}/></Link></section>
  </>;
}
