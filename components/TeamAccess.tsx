...
export function TeamAccess(){
  ...
  const canConfigureMembers=(appId:string)=>{const row=contractedRow(appId);return appId!=='zeus'||['plus','premium'].includes(String(row?.planCode||''));};
  ...
  const changeAccess=async(userId:string,appId:AppId,enabled:boolean,canConfigure:boolean)=>{
    const key=`${userId}:${appId}:access`;setPending(key);clearFeedback();
  ...
  const changeConfigure=async(userId:string,appId:AppId,enabled:boolean)=>{
    const key=`${userId}:${appId}:configure`;setPending(key);clearFeedback();
  ...
  const setInviteAccess=(appId:AppId,enabled:boolean)=>setInviteApps(current=>({...current,[appId]:{access:enabled,configure:enabled?(current[appId]?.configure||false):false}}));
  const setInviteConfigure=(appId:AppId,enabled:boolean)=>setInviteApps(current=>({...current,[appId]:{access:enabled?true:(current[appId]?.access||false),configure:enabled}}));

  const submitInvite=async(event:FormEvent)=>{
    event.preventDefault();clearFeedback();
    const selected=contracted.flatMap(app=>{const choice=inviteApps[app.slug];return choice?.access?[{appId:app.slug as AppId,canConfigure:canConfigureMembers(app.slug)&&choice.configure}]:[];});
    if(!selected.length){setError('Selecione pelo menos um aplicativo para essa pessoa.');return;}
    const blocked=selected.find(item=>full(item.appId));
    if(blocked){const row=contractedRow(blocked.appId);setError(`${apps.find(item=>item.slug===blocked.appId)?.name||'O aplicativo'} já está com ${usage(blocked.appId)} de ${row?.seats||1} acessos utilizados.`);return;}
    setPending('invite');
    try{const result=await access.inviteMember(inviteName,inviteEmail,selected);setInviteName('');setInviteEmail('');setInviteApps({});setInviteOpen(false);setMessage(result.mode==='existing'?'Pessoa adicionada. Ela já pode entrar com a conta que possui.':'Convite enviado por e-mail. A pessoa poderá criar a senha e acessar os aplicativos liberados.');}
    catch(reason){setError((reason as {message?:string}).message||'Não foi possível adicionar essa pessoa.');}finally{setPending('');}
  };
  ...
  return <>
    ...
    {access.isOwner&&inviteOpen&&<form className="team-invite-card" onSubmit={submitInvite}>
      ...
      <div className="team-invite-permissions"><div className="team-invite-permissions-title"><strong>Aplicativos</strong><small>Marque o que essa pessoa poderá usar.</small></div>{contracted.length===0?<p className="team-empty">Nenhum aplicativo ativo para liberar.</p>:contracted.map(app=>{const appId=app.slug as AppId;const choice=inviteApps[appId]||{access:false,configure:false};const row=contractedRow(appId)!;const atLimit=full(appId);return <div className="team-invite-app" key={appId}><div><strong>{app.name}</strong><small>{usage(appId)} de {Math.max(1,row.seats||1)} acessos utilizados</small></div><label className="team-toggle"><input type="checkbox" checked={choice.access} disabled={atLimit&&!choice.access} onChange={event=>setInviteAccess(appId,event.target.checked)}/><span>Acesso</span></label>{canConfigureMembers(appId)&&<label className="team-toggle"><input type="checkbox" checked={choice.configure} disabled={!choice.access} onChange={event=>setInviteConfigure(appId,event.target.checked)}/><span><Settings2 size={14}/> Configurar</span></label>}</div>;})}</div>
    ...
    {error&&<div className="account-form-feedback is-error" role="alert">{error}</div>}{message&&<div className="account-form-feedback is-success" role="status">{message}</div>}
    <section className="team-list">{account.members.map(member=>{const owner=member.role==='owner';return <article className="team-card" key={member.userId}><header className="team-card-header"><div className="team-avatar"><UsersRound size={19}/></div><div><h2>{member.displayName}</h2><p>{owner?'Titular da conta':'Usuário da conta'}</p></div><div className="team-card-header-actions"><span className="team-role">{owner?'Titular':'Usuário'}</span>{access.isOwner&&!owner&&<button className="team-remove" type="button" disabled={pending===`${member.userId}:remove`} onClick={()=>void removeMember(member.userId,member.displayName)} aria-label={`Remover ${member.displayName} da equipe`}><Trash2 size={15}/><span>{pending===`${member.userId}:remove`?'Removendo…':'Remover'}</span></button>}</div></header><div className="team-access-table">{contracted.length===0?<p className="team-empty">Nenhum aplicativo ativo para configurar.</p>:contracted.map(app=>{const appId=app.slug as AppId;const memberAccess=member.apps.find(item=>item.appId===appId);const enabled=owner||!!memberAccess;const canConfigure=owner||!!memberAccess?.canConfigure;const canEdit=access.isOwner&&!owner;const busy=pending.startsWith(`${member.userId}:${appId}`)||pending===`${member.userId}:remove`;return <div className="team-app-row" key={app.slug}><div><strong>{app.name}</strong><small>{usage(appId)} de {Math.max(1,contractedRow(appId)?.seats||1)} acessos</small></div><label className="team-toggle"><input type="checkbox" checked={enabled} disabled={!canEdit||busy||(!enabled&&full(appId))} onChange={event=>void changeAccess(member.userId,appId,event.target.checked,canConfigure)}/><span>Acesso</span></label>{canConfigureMembers(appId)&&<label className="team-toggle"><input type="checkbox" checked={canConfigure} disabled={!canEdit||!enabled||busy} onChange={event=>void changeConfigure(member.userId,appId,event.target.checked)}/><span><Settings2 size={14}/> Configurar</span></label>}</div>;})}</div></article>;})}</section>
  </>;
}
