...
export async function GET(request: NextRequest) {
  ...
  try {
    await requireZeusFeature(access.accountId, 'billing');
    const params: Record<string, string> = { select:'id,job_id,job_number,customer_id,amount_cents,status,payment_method,payment_provider,provider_reference,paid_at,closed_at,notes,created_at,updated_at', tenant_key:`eq.${access.accountId}`, order:'created_at.desc' };
    if (jobId) params.job_id = `eq.${jobId}`;
    if (status) params.status = `eq.${status}`;
    const rows = await operationalRest('zeus', `billing_records?${query(params)}`);
    return NextResponse.json({ records: rows || [] });
  } catch (reason) {
    const planBlocked = reason instanceof Error && reason.message.startsWith('PLAN_FEATURE_REQUIRED:');
    return NextResponse.json({ error: planFeatureError(reason, 'Faturamento está disponível a partir do Zeus Plus.') }, { status: planBlocked ? 403 : 503 });
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  try { await requireZeusFeature(access.accountId, 'billing'); }
  catch (reason) { return NextResponse.json({ error: planFeatureError(reason, 'Faturamento está disponível a partir do Zeus Plus.') }, { status:403 }); }
  ...
  try {
    ...
    const update: Record<string, unknown> = { status, payment_method: typeof body?.paymentMethod === 'string' ? body.paymentMethod.trim().slice(0,120) : '', notes: typeof body?.notes === 'string' ? body.notes.trim().slice(0,2000) : '', updated_at:new Date().toISOString() };
    ...
    const rows = await operationalRest('zeus', `billing_records?${query({ tenant_key:`eq.${access.accountId}`, job_id:`eq.${jobId}` })}`, { method:'PATCH', headers:{Prefer:'return=representation'}, body:JSON.stringify(update) });
