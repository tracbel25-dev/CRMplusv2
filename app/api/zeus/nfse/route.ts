...
import { createInvoiceDraft, markPendingConfiguration, readFiscalWorkspace, saveFiscalProfile } from '@/lib/fiscal/zeusNfse';
  ...
  try {
    ...
    if (action === 'issue') {
      ...
      return NextResponse.json({
        invoice,
        error: 'A emissão direta ainda depende do certificado digital do contribuinte, credenciamento/autorização do município e do conector fiscal correspondente. O registro ficou preservado para concluir a transmissão sem recriar a nota.'
      }, { status: 409 });
    }
