...
async function verifyCaller(request: NextRequest) {
  ...
  try {
    const authResponse = await fetch(`${STORE_SUPABASE.url}/auth/v1/user`, { headers:{ apikey:STORE_SUPABASE.publishableKey, authorization:`Bearer ${token}` }, cache:'no-store' });
...
async function appSeatUsage(service: SupabaseClient, accountId: string, appId: string) {
  ...
  if (!app) return null;
  const { data: members, error: memberError } = await service.from('account_members').select('user_id,role').eq('account_id', accountId).eq('status','active');
  if (memberError) throw memberError;
  const activeIds = new Set((members || []).map(row => String(row.user_id)));
  const ownerIds = new Set((members || []).filter(row => row.role === 'owner').map(row => String(row.user_id)));
  const { data: rows, error } = await service.from('member_app_access').select('user_id').eq('account_id', accountId).eq('app_id', appId);
  if (error) throw error;
  const memberIds = new Set((rows || []).map(row => String(row.user_id)).filter(id => activeIds.has(id) && !ownerIds.has(id)));
  return { limit:Math.max(1, Number(app.seats || 1)), used:ownerIds.size + memberIds.size, memberIds };
}

async function assertSeatAvailable(service: SupabaseClient, accountId: string, appId: string, targetUserId?: string) {
  const usage = await appSeatUsage(service, accountId, appId);
  if (!usage) throw new Error('APP_NOT_ACTIVE');
  if (targetUserId && usage.memberIds.has(targetUserId)) return usage;
  if (usage.used >= usage.limit) {
    const error = new Error(`SEAT_LIMIT:${appId}:${usage.used}:${usage.limit}`);
    throw error;
  }
  return usage;
}
...
async function getTargetMember(service: SupabaseClient, accountId: string, userId: string) {
  const { data, error } = await service.from('account_members').select('user_id, role, status').eq('account_id', accountId).eq('user_id', userId).maybeSingle();
  if (error) throw error;
  return data;
}

async function setSelectedApps(service: SupabaseClient, accountId: string, userId: string, selected: PermissionInput[]) {
  const { error: clearError } = await service.from('member_app_access').delete().eq('account_id',accountId).eq('user_id',userId);
  if (clearError) throw clearError;
  if (!selected.length) return;
  const { error: insertError } = await service.from('member_app_access').insert(selected.map(item => ({ account_id:accountId,user_id:userId,app_id:item.appId,can_configure:!!item.canConfigure,updated_at:new Date().toISOString() })));
  if (insertError) throw insertError;
}

function seatMessage(reason: unknown) {
  const message = reason instanceof Error ? reason.message : '';
  if (!message.startsWith('SEAT_LIMIT:')) return null;
  const [,appId,used,limit] = message.split(':');
  const name = appId === 'zeus' ? 'Zeus' : 'aplicativo';
  return `O ${name} já está com ${used} de ${limit} acessos utilizados. Faça upgrade ou remova um acesso antes de continuar.`;
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    if (body.action === 'invite') {
      ...
      if(targetUser){
        const {data:memberships,error:membershipError}=await service.from('account_members').select('account_id,status,role').eq('user_id',targetUser.id);
        ...
        if(activeMembership&&activeMembership.account_id!==accountId)return responseError(409,'Este e-mail já está vinculado a outra empresa e não pode ser adicionado agora.');
      }
      for (const item of uniqueSelected) await assertSeatAvailable(service,accountId,item.appId,targetUser?.id);

      if(!targetUser){
        const origin=new URL(request.url).origin;
        const {data,error:inviteError}=await service.auth.admin.inviteUserByEmail(email,{redirectTo:`${origin}/auth/confirm`,data:{name,signup_redirect:'/nova-senha?convite=1'}});
        if(inviteError||!data.user){if(inviteError?.message?.toLowerCase().includes('rate'))return responseError(429,'Muitos convites foram enviados em pouco tempo. Aguarde alguns minutos e tente novamente.');throw inviteError||new Error('invite_failed');}
        targetUser=data.user;createdByInvite=true;
      }
      try{
        const {error:profileError}=await service.from('profiles').upsert({user_id:targetUser.id,display_name:name,updated_at:new Date().toISOString()},{onConflict:'user_id'});if(profileError)throw profileError;
        const existing=await getTargetMember(service,accountId,targetUser.id);
        if(existing){const {error}=await service.from('account_members').update({role:'member',status:'active'}).eq('account_id',accountId).eq('user_id',targetUser.id);if(error)throw error;}
        else{const {error}=await service.from('account_members').insert({account_id:accountId,user_id:targetUser.id,role:'member',status:'active'});if(error)throw error;}
        await setSelectedApps(service,accountId,targetUser.id,uniqueSelected);
      }catch(writeError){if(createdByInvite)await service.auth.admin.deleteUser(targetUser.id).catch(()=>undefined);throw writeError;}
      return NextResponse.json({ok:true,mode:createdByInvite?'invited':'existing'});
    }

    if(body.action==='remove'){
      const targetId=body.userId?.trim()||'';if(!targetId)return responseError(400,'Selecione a pessoa que deseja remover.');
      const target=await getTargetMember(service,accountId,targetId);if(!target||target.status!=='active')return responseError(404,'Essa pessoa não faz mais parte da equipe.');if(target.role==='owner')return responseError(400,'O titular da conta não pode ser removido da própria equipe.');
      const {error:accessError}=await service.from('member_app_access').delete().eq('account_id',accountId).eq('user_id',targetId);if(accessError)throw accessError;
      const {error:memberError}=await service.from('account_members').update({status:'suspended'}).eq('account_id',accountId).eq('user_id',targetId);if(memberError)throw memberError;
      return NextResponse.json({ok:true});
    }

    const targetId=body.userId?.trim()||'';const appId=body.appId?.trim()||'';
    if(!targetId||!appId)return responseError(400,'Não foi possível identificar a pessoa ou o aplicativo.');
    const target=await getTargetMember(service,accountId,targetId);if(!target||target.status!=='active')return responseError(404,'Essa pessoa não faz mais parte da equipe.');if(target.role==='owner')return responseError(400,'O titular já possui acesso completo aos aplicativos ativos.');
    ...
    const limit=seatMessage(reason);if(limit)return responseError(409,limit);
    console.error('team-management-failed',reason);return responseError(500,'Não foi possível concluir a alteração da equipe. Tente novamente.');
  }
}
