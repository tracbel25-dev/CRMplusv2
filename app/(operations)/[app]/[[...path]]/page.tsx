...
export default async function OperationPage({params}:{params:Promise<{app:string;path?:string[]}>}){const {app,path=[]}=await params;if(!isApp(app)||path.length>1)notFound();const page=path[0]||'inicio';if(page==='login'||page==='cadastro')redirect(`/${app}`);if(!appPages(app).includes(page))notFound();return <AppRuntime key={app} app={app} page={page}/>}
