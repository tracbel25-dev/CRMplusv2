...
export async function getPublicPlans(): Promise<PublicPlan[]> {
  try {
    const response = await fetch(`${STORE_SUPABASE.url}/rest/v1/plans?select=id,app_id,plan_code,seats,billing_interval,amount_cents,currency&active=eq.true&order=amount_cents.asc`, {
