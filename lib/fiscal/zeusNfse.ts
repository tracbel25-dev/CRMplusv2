...
function zeusAdmin() {
  const config = readOperationalSupabaseConfig('zeus');
  if (!config.secretKey) throw new Error('ZEUS_SUPABASE_SECRET_KEY não está configurada no servidor.');
...
export async function readFiscalWorkspace(tenantKey: string) {
  ...
  const [profile, invoices] = await Promise.all([
    ...
    db.from('service_invoices').select('id,job_id,customer_name,customer_document,service_code,description,amount_cents,status,provider,provider_reference,access_key,invoice_number,error_message,issued_at,created_at').eq('tenant_key', tenantKey).order('created_at', { ascending: false }).limit(50),
  ]);
...
export async function createInvoiceDraft(tenantKey: string, userId: string, body: Record<string, unknown>) {
  ...
  const row = {
    tenant_key: tenantKey,
    job_id: clean(body.jobId, 64) || null,
