...
export function validateZeusPlanTransition(current: Data | null, next: Data, plan: ZeusPlanCode) {
  ...
  if (!zeusHasFeature(plan, 'scheduling') && stable(current.appointments) !== stable(next.appointments)) {
    throw new Error('PLAN_FEATURE_REQUIRED: agendamentos');
  }

  if (!zeusHasFeature(plan, 'budgets')) {
    if (stable(current.quotes) !== stable(next.quotes)) throw new Error('PLAN_FEATURE_REQUIRED: orçamentos');
